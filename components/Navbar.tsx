...
const Navbar: React.FC<NavbarProps> = ({ onNavigate, currentPage }) => {
  ...
  return (
    <nav className={`fixed w-full z-[100] transition-all duration-500 ${scrolled || currentPage !== 'home' ? 'bg-[var(--bg-deep)]/95 backdrop-blur-lg py-3 border-b border-[var(--border)] shadow-2xl' : 'bg-transparent py-8'}`}>
      <div className="container mx-auto px-6 flex justify-between items-center">
        <button onClick={() => onNavigate('home')} className="flex items-center space-x-4 group cursor-pointer outline-none">
          <img 
